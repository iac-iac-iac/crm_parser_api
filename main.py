...
def main():
    ...
    # Инициализация
    api_client = DataMasterClient(api_url, api_token)
...
def parse_args():
    ...
    # Новое: продолжение прерванного сбора
...
def main():
    ...
    # Обычный сбор данных
    api_client = DataMasterClient(api_url, api_token)
    orchestrator = CollectionOrchestrator(api_client, db, rate_limit)

    logger.info(
        f"Starting collection (limit_clients={args.limit_clients}, "
        f"limit_projects={args.limit_projects}, max_pages={args.max_pages})"
    )
